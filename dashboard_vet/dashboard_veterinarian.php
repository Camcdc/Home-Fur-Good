<?php
// Ensures user is logged in and is vet
session_start();

// Ensures user is logged in and is a vet
if (!isset($_SESSION['userID']) || strtolower($_SESSION['role']) !== 'veterinarian') {
    echo "<script>
...
// session_start();
// $vetName = $_SESSION['vet_name'] ?? 'Veterinarian';
...
$recentReportsListQuery = "SELECT r.id, a.name AS animal_name, r.created_at FROM medical_report r JOIN animal a ON r.animal_id = a.id ORDER BY r.created_at DESC LIMIT 5";
        ...
        <div class="container">
            ...
            <div class="stat-cards">
                ...
                <div class="card">
                    ...
                    <p><strong><?php echo $todaysAppointments; ?></strong></p>
                </div>
                <div class="card">
            ...
            <div class="dashboard-section">
                <div class="section">
                    <div class="section-header">
                        <span><i class="fa-solid fa-bell"></i> Alerts & Follow-ups</span>
                    </div>
                    <div class="section-list">
                        <?php if ($followUpResult && mysqli_num_rows($followUpResult) > 0): ?>
                            <?php while ($row = mysqli_fetch_assoc($followUpResult)): ?>
                                <div class="list-item">
                                    <div class="item-title"><?php echo htmlspecialchars($row['name']); ?> (<?php echo htmlspecialchars($row['species']); ?>)</div>
                                    <div class="item-date">Needs follow-up</div>
                                </div>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <div class="list-item">No animals need follow-up.</div>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="section">
                    ...
                    <div class="section-list">
                        <?php if ($recentAnimalsListResult && mysqli_num_rows($recentAnimalsListResult) > 0): ?>
                            <?php while ($row = mysqli_fetch_assoc($recentAnimalsListResult)): ?>
                                <div class="list-item">
                                    <div class="item-title"><?php echo htmlspecialchars($row['name']); ?> (<?php echo htmlspecialchars($row['species']); ?>)</div>
                                    <div class="item-date">Registered: <?php echo htmlspecialchars($row['registration_date']); ?></div>
                                </div>
                            <?php endwhile; ?>
                        <?php else: ?>
                    ...
                    <div class="section-list">
                        <?php if ($recentReportsListResult && mysqli_num_rows($recentReportsListResult) > 0): ?>
                            <?php while ($row = mysqli_fetch_assoc($recentReportsListResult)): ?>
                                <div class="list-item">
                                    <div class="item-title">Report #<?php echo htmlspecialchars($row['id']); ?> for <?php echo htmlspecialchars($row['animal_name']); ?></div>
                                    <div class="item-date">Created: <?php echo htmlspecialchars($row['created_at']); ?></div>
                                </div>
                            <?php endwhile; ?>
                        <?php else: ?>
...
<h1>Welcome to the Vet Dashboard</h1>
<p>This page is only accessible by veterinarians.</p>
