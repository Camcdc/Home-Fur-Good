...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
            ...
            // If approving a foster application, update the animal status to 'Foster'
            if ($action === 'Approved') {
               $stmt2 = $conn->prepare("UPDATE animal SET status = 'Fostered' 
                            WHERE animalID = (SELECT animalID FROM application WHERE applicationID = ?)");
                if ($stmt2) {
                    $stmt2->bind_param("i", $applicationID);
                    $stmt2->execute();
                    $stmt2->close();
                } else {
                    error_log("Failed to prepare approve statement: " . $conn->error);
                }
            }
                                     
            // If reverting to pending or rejecting, update animal status back to 'Available'
            if ($action === 'Pending' || $action === 'Rejected') {
                $stmt3 = $conn->prepare("UPDATE animal a
                                       INNER JOIN application app ON a.animalID = app.animalID
                                       SET a.status = 'Available'
                                       WHERE app.applicationID = ? AND app.applicationType = 'Fostered'");
                if ($stmt3) {
                    $stmt3->bind_param("i", $applicationID);
                    $stmt3->execute();
                    $stmt3->close();
                } else {
                    error_log("Failed to prepare revert/reject statement: " . $conn->error);
                }
            }
            ...
            $conn->rollback();
            $conn->autocommit(TRUE);
            error_log("Error updating foster application: " . $e->getMessage());
        }
    }
    header("Location: manageFosters.php");
    exit;
}
...
$sql = "SELECT a.applicationID, a.applicationStatus, u.fname, u.sname, a.applicationDate
        ...
        WHERE a.applicationType = 'Foster' $where_clause
...
<head>
    <meta charset="UTF-8">
    <title>Manage Foster Applications</title>
...
<body>
    <div class="page-heading">
        <a href='foster_admin.php' class='updateBtn'>← Back to Dashboard</a>
        <h1>Manage Foster Applications</h1>
    </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php endif; ?>
                        <a href="view_foster_application_details.php?id=<?php echo $row['applicationID']; ?>" class="view-details-link">View Details</a>
                    </td>
                </tr>
                <?php endwhile; ?>
            </tbody>
    ...
    <?php else: ?>
    <p style="padding-left: 300px;">No foster applications found.</p>
    <?php endif; ?>
